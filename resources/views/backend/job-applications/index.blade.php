                        ...
                        <tr class="border-b last:border-b-0">
                            ...
                            <td class="px-4 py-3">
                                @switch($application['status'])
                                    @case('pending')
                                        ...
                                        @break
                                    @case('shortlisted')
                                        <span class="px-2 py-1 text-xs rounded bg-yellow-200 text-yellow-800">Shortlisted</span>
                                        @break
                                    @case('interview_scheduled')
                                        <span class="px-2 py-1 text-xs rounded bg-blue-200 text-blue-800">Interview Scheduled</span>
                                        @break
                                    @case('interviewed')
                                        <span class="px-2 py-1 text-xs rounded bg-purple-200 text-purple-800">Interviewed</span>
                                        @break
                                    @case('hired')
                                        <span class="px-2 py-1 text-xs rounded bg-green-200 text-green-800">Hired</span>
                                        @break
                                @endswitch
                            </td>
                            ...
                            <td class="px-4 py-3">
                                <div x-data="{ status: '{{ old('status', $application['status']) }}' }">
                                    <form action="{{ route('admin.job-applications.update', $application['id']) }}" method="POST" class="space-y-2">
                                        @csrf
                                        @method('PUT')
